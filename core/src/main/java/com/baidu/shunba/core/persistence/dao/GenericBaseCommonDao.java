...
import com.baidu.shunba.common.utils.MyBeanUtils;
import com.baidu.shunba.common.utils.PagerUtil;
import com.baidu.shunba.common.utils.SUReflectUtils;
import com.baidu.shunba.common.utils.StringUtil;
import com.baidu.shunba.common.utils.ToEntityUtil;
import com.baidu.shunba.common.utils.oConvertUtils;
import com.baidu.shunba.core.entity.JeecgEntityTitle;
import com.baidu.shunba.core.persistence.base.CriteriaQuery;
import com.baidu.shunba.core.persistence.base.DetachedCriteriaUtil;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	 ...
	 * 初始化Log4j的一个实例
	 */
	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(GenericBaseCommonDao.class);
	...
	public <T> void batchSave(Collection<T> entitys) {
		...
		int i = 0;
		for (Iterator<T> it = entitys.iterator(); it.hasNext(); i++) {
	...
	public Long getRowCount(CriteriaQuery cq) {
		Criteria criteria = cq.getDetachedCriteria().getExecutableCriteria(getSession());
		return oConvertUtils.getLong(((Criteria) criteria.setProjection(Projections.rowCount())).uniqueResult(), 0);
	}
	...
	public PageList getPageList(final CriteriaQuery cq, final boolean isOffset) {
		...
		return new PageList(criteria.list(), offset, curPageNO, allCounts);
	}
	...
	public DataGridReturn getDataGridReturn(final CriteriaQuery cq, final boolean isOffset) {
		...
		int allCounts = 0;
		try {
			allCounts = ((Long) criteria.setProjection(Projections.rowCount()).uniqueResult()).intValue();
		} catch (Exception e) {
		}
		criteria.setProjection(projection);
		if (projection == null) {
		...
		if (StringUtils.isNotEmpty(cq.getDataGrid().getSort())) {
	...
	public PageList getPageList(final HqlQuery hqlQuery) {
		...
		} else if (hqlQuery.getParamMap() != null && hqlQuery.getParamMap().size() > 0) {
			for (String key : hqlQuery.getParamMap().keySet()) {
				Object value = hqlQuery.getParamMap().get(key);
				if (value instanceof Collection) {
		...
		query.setFirstResult(offset);
		query.setMaxResults(hqlQuery.getPageSize());
		return new PageList(query.list(), offset, curPageNO, allCounts);
	}
...
//		return this.jdbcTemplate.queryForLong(sql, objs);
		return (Long) this.jdbcTemplate.queryForObject(sql, Long.class, objs);
	}
	...
	/**
	 * 查询对象列表，例如 queryForObjectListJdbc(sql, args, ErpEmployee.class);
	 * 
	 * @param sql
	 * @param args
	 * @param requiredType
	...
	public <T> List<T> queryForObjectListJdbc(String sql, Object[] args, Class<T> requiredType) {
		return this.jdbcTemplate.query(sql, args, BeanPropertyRowMapper.newInstance(requiredType));
	}
...
//		return this.jdbcTemplate.queryForInt(sql, param);
		return (Integer) this.jdbcTemplate.queryForObject(sql, Integer.class, param);
	}
